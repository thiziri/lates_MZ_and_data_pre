...
def extend_word_embedding(w2v_file, out_dir):
    ...
    try:
        ...
        try:
            ...
            f = open(w2v_file, 'rb')
            lines = f.readlines() # try reading
            float(lines[0].rstrip().split(b" ")[1])
            binary_lines = True
        except:
            print("Reading as GoogleNews-vectors-negative300.bin model ...")
    if "google" in w2v_file.lower():
        from gensim.models.keyedvectors import KeyedVectors
        model = KeyedVectors.load_word2vec_format(w2v_file, binary=True)
        model.save_word2vec_format(w2v_file+".temp.txt", binary=False)
        del model
        f = io.open(w2v_file, encoding="utf8")
        lines = f.readlines() # try reading
        temp = True
        w2v_file = w2v_file+".temp.txt"
    ...
    print(len(lines))
    #print(lines[:5])
    print("Extention: this may take time depending on the embedding size ...")

    for l in tqdm(lines):
        # Explicitly splitting on " " is important, so we don't
        # get rid of Unicode non-breaking spaces in the vectors.
        #print(idx)
        entries = l.rstrip().split(b" " if binary_lines else " ")
