...
class ConvWeakCollaboration(BasicModel):
    ...
    def build(self):
        ...
        # ############# permutation ######################
        input_mat = Permute((3, 2, 1))(input_mat)
        show_layer_info('permuted Match', input_mat)
        ...
        show_layer_info('permuted Conv1D', merged)
        merged = MaxPooling1D(pool_size=self.config['pool_size'], name="maxPool1")(merged)
        show_layer_info('MaxPooling1D', merged)

        merged = Conv1D(int(self.config['filters']/self.config['pool_size']), self.config['kernel_size'],
