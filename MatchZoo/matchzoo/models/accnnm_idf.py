...
def get_idf(idf_path, vocab_size):
    # get the idf values as an embedding weights of dimension 1
    embed_dict = read_embedding(filename=idf_path)
    _PAD_ = vocab_size - 1
    embed_dict[_PAD_] = np.zeros((1,), dtype=np.float32)
    embed = np.float32(np.random.uniform(-0.2, 0.2, [vocab_size, 1]))
    idf = convert_embed_2_numpy(embed_dict, embed=embed)
    return idf
...
class A_CCNNM(BasicModel):
    def __init__(self, config):
        ...
        self.check_list = ['text1_maxlen', 'text2_maxlen', 'embed', 'embed_size', 'train_embed',  'vocab_size',
                           ...
                           "text1_attention", "text2_attention", "merge_levels", "word_idf"]
    ...
    def build(self):
        ...
        if self.config['context_embed'] > 1:  # compute context importance weights
            ...
            if self.config["bidirectional"]:
                ...
                contxt = LSTM(lstm_units, return_sequences=False)(contxt)
            contxt = BatchNormalization()(contxt)
            contxt = Dropout(self.config['lstm_dropout_rate'])(contxt)
            show_layer_info('LSTM', contxt)
        else:
        ...
        # ############################## add idf query word gating:
        if self.config["idf_gating"]:
            # load idf values:
            idf = get_idf(self.config['word_idf'], self.config['vocab_size'])
            embed_idf = Embedding(self.config['vocab_size'], 1, weights=[idf], trainable=False)
            q_idf = embed_idf(query)
            show_layer_info('embed_idf', q_idf)
            q_w = Dense(1, kernel_initializer=self.initializer_gate, use_bias=False)(q_idf)
            show_layer_info('Dense', q_w)
            q_w = Lambda(lambda x: softmax(x, axis=1), output_shape=(self.config['text1_maxlen'],))(q_w)
            show_layer_info('Lambda-softmax', q_w)
            ...
            # apply the gatings
            contxt = Dense(self.config['text1_maxlen'])(contxt)
            show_layer_info('To_gating', contxt)
            contxt = Dot(axes=[1, 1])([contxt, q_w])
            show_layer_info('final_gates', contxt)
        ...
        plot_model(model, to_file='images/idf_accnnm_ec_'+str(self.config['context_embed'])+".png", show_shapes=True,
